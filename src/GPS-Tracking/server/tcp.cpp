...
std::string IMEI_JSON_LOCATION =
    "/home/" + core::config::getUsername() + "/" + IMEI_JSON_FILENAME;
...
void setTcpConfig(json setTcpConfig) { config = setTcpConfig; }
...
void newClient(int socket) {
  ...
  // if (close(socket) < 0) {
  //   std::cout << "\x1b[Failed to close socket: \x1b[0m" << socket <<
  //   std::endl; failed_socket.push_back(socket); failed_count++;
  // }
...
void tcpServer() {
  ...
  for (;;) {
    ...
    // If master_socket is still set to the file descriptor, then it's an
    // incoming connection
    ...
    for (auto it : failed_socket) {
      if (close(it) == 0) {
        failed_socket.erase(
            std::find(failed_socket.begin(), failed_socket.end(), it));
      }
    }
