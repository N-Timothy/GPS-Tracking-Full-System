...
public:
  bool getRealTimeState() { return this->realTimeState; }

  void setRealTimeState(bool newState) { this->realTimeState = newState; }
  ...
  void sendGPRSCommand(int connfd, bool realTimeFlag) {
    ...
    byte realtime_command[] = {
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x28, 0x0c, 0x01, 0x05,
        0x00, 0x00, 0x00, 0x20, 's',  'e',  't',  'p',  'a',  'r',  'a',
        'm',  ' ',  '1',  '0',  '0',  '5',  '0',  ':',  '5',  ';',  '1',
        '0',  '1',  '5',  '0',  ':',  '5',  ';',  '1',  '0',  '2',  '5',
        '0',  ':',  '5',  0x01, 0x00, 0x00, 0x99, 0x74};

    // setparam 10050:300;10150:300;10250:300
    byte normal_command[] = {
        0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x2e, 0x0c, 0x01, 0x05, 0x00,
        0x00, 0x00, 0x26, 's',  'e',  't',  'p',  'a',  'r',  'a',  'm',  ' ',
        '1',  '0',  '0',  '5',  '0',  ':',  '3',  '0',  '0',  ';',  '1',  '0',
        '1',  '5',  '0',  ':',  '3',  '0',  '0',  ';',  '1',  '0',  '2',  '5',
        '0',  ':',  '3',  '0',  '0',  0x01, 0x00, 0x00, 0xb7, 0x14};

    if (realTimeFlag)
      send(connfd, (char *)&realtime_command, sizeof(realtime_command), 0);
    else
      send(connfd, (char *)&normal_command, sizeof(normal_command), 0);
  }
};

void toRealTime(std::string imei, bool toggle) {
  if (toggle)
    imeiRealTimeVec.push_back(imei);
  else
    imeiNormalVec.push_back(imei);
}

std::vector<std::string> removeImei(std::vector<std::string> vector,
                                    std::string imei) {
...
int communicate(int connfd) {
  ...
  for (;;) {
    ...
    if (FD_ISSET(connfd, &readfds)) {
      ...
      if (!cv.wait_until(lk, std::chrono::system_clock::now() + 1s,
                         [] { return ready; })) {
        return -6;
      } else {
        try {
